...
- **Body (dos modos):** identidad `handle` y/o `subscriber_id` (+ `lead` opcional). **Modo A** `{ modelo: "<texto>" }` · **Modo B (quiz)** `{ motorizacion, disciplina, presupuesto, altura, talla }` (todos opcionales). **La talla se asigna por `altura`** ("1,75", "175", "175 cm" → metros): se cruza contra el `Rango altura` de cada bici (dentro del rango suma, fuera penaliza, bici sin rango = neutra). `talla` explícita sigue soportada. El criterio queda en `Crit · altura` del Interés.
...
- **Escribe:** **Solicitud** → Modelo buscado, Talla, Presupuesto, Disciplina, Motorización, Notas, Contacto, `Estado=Llamada pendiente` (el staff llama a confirmar detalles y lo pasa a `Buscando`), `Origen=Bot DM`, Fecha, link `Lead`. **Lead** → `WhatsApp` + `Opt-in WhatsApp` + `Fecha opt-in` + `MC subscriber id`. **Interés** → el `No-match` más reciente pasa a `Encargo`=✓ (marca de embudo, best-effort).
- **Devuelve:** `{ ok, encargo, solicitudId, leadId, leadCreado, interesId, modeloBuscado }`.
- **Cola de sourcing = tabla `Solicitudes`** (Estado `Llamada pendiente → Buscando → Conseguida → Cerrada`): el staff la trabaja desde la interfaz (cards) y crea tickets manuales por formulario en la misma tabla (`Origen=Manual`). Además de recuperar al lead, dice **qué bicis salir a conseguir**. Aviso al conseguirla: manual hoy; plantilla `reactivacion_stock` a futuro.
...
### `mc-llamado` — `POST /api/mc-llamado` ✅ (Puertas 1 y 2 · leads de región)
Cuando la persona **no está en Santiago**, en vez de visita se agenda una **llamada** del equipo. La pregunta "¿Estás en Santiago?" vive en la cadena compartida de agenda de ambas puertas (todo camino a agendar pasa por ahí).
- **Body:** `{ handle?, subscriber_id?, telefono?, optin?, ciudad?, franja?, bici?, reel?, notas? }`. La bici de interés viaja como dato: `bici` = record id (`cf_hero_bici`, Puerta 2) o `reel` = Post ID Instagram (Puerta 1 → resuelve vía `Reels.Bici`, igual que mc-evento). Ignora merge tags sin resolver.
- **Escribe:** ticket en **`Llamados`** (`tblgApNKo9YiqPalw`: Nombre, Teléfono, Ciudad, Franja Mañana/Tarde, Bici de interés, `Estado Llamada pendiente→Llamado→Cerrada`, Origen, Fecha, link Lead) + teléfono/opt-in en el Lead. **Clave: NO escribe `Fecha visita`** (no dispara recordatorios de visita). El ticket nace en **"Llamada pendiente"** = cola de llamados del staff; tras llamar se mueve a `Llamado`/`Cerrada`.
