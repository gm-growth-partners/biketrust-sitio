...
export default {
  async scheduled(event, env, ctx) {
    ...
    const sourcUrl = env.CRON_URL.replace('cron-recordatorios', 'cron-sourcing');
    for (const [name, url] of [['recordatorios', recUrl], ['briefing', briefUrl], ['reenganche', reengUrl], ['sourcing', sourcUrl]]) {
      try {
