...
- 📤 **`/bicis.json` — manifiesto público del catálogo (2026-08-18):** lo escribe `bicisJSON()` en cada build, con `ref` · `slug` · marca · modelo · talla · año · disciplina · `electrica` · precio · puntaje · `estado` (Disponible/Reservada/Vendida) · `foto` (URL absoluta de la portada) · `fotos` (cuántas) · `ficha` (URL absoluta). **Lo consume la pestaña «Inventario» del tablero.** 🔴 **Existe porque el slug y la foto NO se pueden reconstruir fuera de este build:** el slug se desambigua por ORDEN del catálogo (`-2`, `-3`) y la extensión de la portada depende del archivo de Airtable — y las URLs de adjuntos de Airtable **expiran**, así que hornearlas en otro repo da fotos rotas en horas. Acá el dato es el real porque lo acaba de escribir `assignSlugs` + `resolveBikePhotos`. No expone nada que no esté ya en `/catalogo`. ⚠️ Si se cambian estas claves, se rompe la pantalla del tablero en silencio (queda «sin datos»).
