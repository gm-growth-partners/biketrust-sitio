...
export async function avisarStaff(env = {}, { cual, flowEnv, campo, texto, extra, sids } = {}) {
  ...
  let enviados = 0;
  const errores = [];
  // A QUIÉN le llegó de verdad, no sólo cuántos. Lo necesita `avisar()` para
  // decidir si se puede sellar: que reciba un observador no descuenta la cola.
  const enviadosA = [];
  ...
  for (const sid of destinos) {
    try {
      ...
      enviados++;
      enviadosA.push(String(sid));
    } catch (e) {
      errores.push(`${sid}: ${String((e && e.message) || e).slice(0, 140)}`);
    }
  }

  return { enviados, enviadosA, errores, motivo: enviados ? 'enviado' : 'error' };
}
...
// ═════════════════════════════════════════════════════════════════════════════
// QUIÉN PUEDE SELLAR (2026-08-20, cazado por la revisión post-despliegue)
//
// El sello `Aviso equipo enviado` significa «el caso está cubierto, no lo vuelvas
// a mandar». Se escribía en cuanto CUALQUIERA recibía el aviso — y eso, con el
// equipo real, resultó estar mal.
//
// Gabriel y Roberto reciben los avisos de llamadas todos los días de 8 a 20 para
// mirar el negocio, pero NO llaman. Un lead que entra un domingo a las 13:00 les
// llegaba a ellos, el ticket quedaba sellado, y el lunes le aparecía a Luis
// marcado «esperando 20h» y ORDENADO DESPUÉS de los que nadie había visto. O sea
// que los leads más frescos quedaban sepultados justo bajo los más viejos.
// Son ~16 horas por semana, y son las de más tráfico orgánico: el fin de semana.
//
// La regla nueva: para los avisos DE CARA AL CLIENTE, el sello exige que se haya
// enterado alguien que **atiende clientes**. Que lo reciba un observador está
// bien —es lo que pidió Gabriel— pero no descuenta la cola.
//
// ⚠️ Sólo aplica a `llamada` y `humano`. En `solicitud`, `consigna` y `sourcing`
// el que actúa es justamente Roberto, que no atiende clientes: exigirle el flag
// ahí dejaría esas colas sin sellar nunca y el barrido reenviaría para siempre.
const EXIGEN_ATIENDE = new Set(['llamada', 'humano']);
...
export async function destinatarios(env = {}, tipo, now = new Date(), { ignorarHorario = false } = {}) {
  ...
  if (suscritos.length) {
    ...
    // `atienden` = los que además pueden SELLAR (ver EXIGEN_ATIENDE). Para los
    // tipos que no lo exigen, cualquiera que reciba sella, así que la lista es
    // la misma que `sids`.
    const puedenSellar = EXIGEN_ATIENDE.has(tipo) ? dentro.filter(p => p.atiende) : dentro;
    return {
      sids: dentro.map(p => p.sid),
      atienden: puedenSellar.map(p => p.sid),
      fuente: 'equipo',
      personas: dentro.map(p => p.nombre),
      soloObservan: dentro.filter(p => !puedenSellar.includes(p)).map(p => p.nombre),
  ...
  // Fallback (sin tabla `Equipo`): no hay forma de distinguir quién atiende, así
  // que cualquiera sella — exactamente como antes de que existiera la tabla.
  const sids = sidsAviso(env, T.cual);
  const abierto = sinFiltro || porDia || SIN_FRANJA_EN_FALLBACK.has(tipo) || enFranja(env, now);
  return { sids: abierto ? sids : [], atienden: abierto ? sids : [], fuente: 'env',
    personas: [], soloObservan: [], fuera: abierto ? [] : sids, sinSid: [] };
}
...
// ¿Hay alguien que además pueda SELLAR (o sea, que vaya a actuar)?
// El barrido lo usa para no correr una cola cuyo aviso sólo verían observadores:
// sin esto les mandaría el mismo WhatsApp cada 15 minutos, sin sellar nunca.
export async function hayQuienActue(env = {}, tipo, now = new Date()) {
  return ((await destinatarios(env, tipo, now)).atienden || []).length > 0;
}

// Los dos motivos que significan «el caso sigue abierto y hay que volver a
// intentarlo», por oposición a un fallo. No gastan intentos y NO sellan: los
// recoge el barrido en el próximo turno, o el briefing de la mañana.
export const quedaPendiente = (motivo) =>
  motivo === 'fuera_de_horario' || motivo === 'solo_observadores';
...
//   puedeSellar = true          → se enteró alguien que ACTÚA → SELLAR.
//   enviados > 0 pero puedeSellar = false → sólo lo vieron observadores: se les
//                                 mandó (lo pidieron) pero la cola NO se descuenta.
...
export async function avisar(env = {}, { tipo, flowEnv, campo, texto, extra, now = new Date(), ignorarHorario = false } = {}) {
  const T = TIPOS[tipo];
  if (!T) return { enviados: 0, puedeSellar: false, errores: [], motivo: `tipo_desconocido:${tipo}`, destinatarios: null };
  const d = await destinatarios(env, tipo, now, { ignorarHorario });
  if (!d.sids.length) {
    const motivo = d.fuera.length ? 'fuera_de_horario' : 'no_configurado';
    return { enviados: 0, puedeSellar: false, errores: [], motivo, destinatarios: d,
      ...(motivo === 'no_configurado' ? { falta: [`sids:${tipo}`] } : {}) };
  }
  const r = await avisarStaff(env, { cual: T.cual, flowEnv, campo, texto, extra, sids: d.sids });
  const conAccion = new Set(d.atienden || d.sids);
  const puedeSellar = (r.enviadosA || []).some(s => conAccion.has(s));
  // Le llegó a alguien, pero sólo a quien mira sin actuar: el caso sigue abierto.
  const motivo = r.enviados > 0 && !puedeSellar ? 'solo_observadores' : r.motivo;
  return { ...r, puedeSellar, motivo, destinatarios: d };
}
