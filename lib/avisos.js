...
// Y estos dos ignoran el horario TAMBIÉN con la tabla `Equipo` puesta:
//   · `briefing` tiene su propia hora (9:00) — filtrarlo por turno lo apagaría.
//   · `reagendo` avisa que una visita de HOY se movió, y es el único aviso del
//     sistema que NO deja sello en ninguna parte: sin red, silenciarlo es
//     perderlo. Si algún día se le pone sello, sale de esta lista.
const IGNORAN_HORARIO = new Set(['briefing', 'reagendo']);
...
export async function destinatarios(env = {}, tipo, now = new Date(), { ignorarHorario = false } = {}) {
  ...
  const sinFiltro = ignorarHorario || IGNORAN_HORARIO.has(tipo);
  ...
  if (suscritos.length) {
    ...
    const dentro = sinFiltro ? conSid : conSid.filter(p => enHorarioPersona(p.horario, now, env));
    return {
  ...
  const abierto = sinFiltro || SIN_FRANJA_EN_FALLBACK.has(tipo) || enFranja(env, now);
