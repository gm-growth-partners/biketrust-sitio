...
//
// EL HORARIO NO SE DECIDE ACÁ DOS VECES: se usa la misma `enFranja()` que el
// barrido. Dentro de 9–20 avisa al tiro; fuera, el sello queda vacío y lo recoge
// el briefing de la mañana — que es exactamente lo que pidió Gabriel.
//
// Body: { subscriber_id?, handle? }   (ManyChat manda los dos; basta uno)
// Protegido por MC_KEY (?key=).

import { enFranja, avisarStaff, unaLinea, chileHora } from '../../lib/avisos.js';
...
async function derivar(env, texto) {
  return avisarStaff(env, {
    cual: 'equipo', flowEnv: 'FLOW_NS_AVISO_EQUIPO', campo: 'cf_aviso_datos', texto: unaLinea(texto),
  });
}
...
export async function onRequestPost({ request, env }) {
  ...
  for (const formula of [
    ...
    if (rec) { leadId = rec.id; leadNombre = rec.fields?.Nombre || rec.fields?.Lead || ''; break; }
  }
  if (!leadId) {
    await derivar(env, `${quien} apretó «Sí, llámenme» pero no tiene ficha en el CRM. Búscalo en ManyChat y llámalo.`);
  ...
  const lr = await afetch(`${api(LLAM)}?pageSize=50&filterByFormula=${encodeURIComponent(`FIND('${esc(leadId)}', ARRAYJOIN({Lead}))`)}`, { headers: rH });
  const tickets = lr.ok ? ((await lr.json()).records || []) : [];
  tickets.sort((a, b) => String(b.createdTime).localeCompare(String(a.createdTime)));
  const t = tickets[0];

  if (!t) {
    await derivar(env, `${leadNombre || quien} apretó «Sí, llámenme» pero no tiene ningún ticket abierto. Créaselo y llámalo.`);
  ...
  if (salida && salida !== 'No contestado') {
    await derivar(env, `${leadNombre || quien} apretó «Sí, llámenme», pero su ticket ya está en «${salida}». No lo moví: revísalo tú.`);
    return reply({ ok: true, accion: 'derivado_a_humano', motivo: 'ticket_ya_avanzado', salida, llamadoId: t.id, leadId });
  }
  if (estado === 'Cerrada') {
    await derivar(env, `${leadNombre || quien} apretó «Sí, llámenme» sobre un ticket ya cerrado. Revísalo tú.`);
  ...
  const enfriando = selloPrevio && (Date.now() - selloPrevio) < REARME_MIN * 60000;
  const dentro = enFranja(env, now);
  ...
  if (agotado) {
    await derivar(env, `${leadNombre || quien} pidió que lo llamaran por ${reaperturas + 1}ª vez. Ya no se le manda más rescate automático: llámalo tú.`);
  }
